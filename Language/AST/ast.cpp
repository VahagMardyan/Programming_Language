...
void VariableNode::print(std::string prefix, bool isLast) const {
    std::cout << prefix << (isLast ? "└── " : "├── ") << "Var (offset: " << offset << ")" << std::endl;
}
...
void UnaryOpNode::print(std::string prefix, bool isLast) const {
    std::string sign = (op == "-" || op == "_") ? "-" : (op == "not" ? "not" : "+");
...
void FunctionCallStatementNode::print(std::string prefix, bool isLast) const {
    call->print(prefix, isLast);
}

void StringNode::print(std::string prefix, bool isLast) const {
